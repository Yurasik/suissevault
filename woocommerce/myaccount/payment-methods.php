			...
			<?php if ( $has_methods ) : ?>
				<div class="cabinet_head grid _four">
					<?php foreach ( wc_get_account_payment_methods_columns() as $column_id => $column_name ) : ?>
						<div class="cabinet_col woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $column_id ); ?> payment-method-<?php echo esc_attr( $column_id ); ?>"><?php echo esc_html( $column_name ); ?></div>
					<?php endforeach; ?>
				</div>
								...
								<div class="cabinet_col woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $column_id ); ?> payment-method-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
									...
									elseif ( 'actions' === $column_id ) {
										foreach ( $method[ 'actions' ] as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
											echo '<a href="' . esc_url( $action[ 'url' ] ) . '" class="action_btn button ' . ' _' . $key . ' ' . sanitize_html_class( $key ) . '">' . esc_html( $action[ 'name' ] ) . '</a>';
										}
									} ?>
								</div>
							<?php endforeach; ?>
